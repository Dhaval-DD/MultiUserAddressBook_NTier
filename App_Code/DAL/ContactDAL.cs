                ...
                using (SqlCommand objCmd = objConn.CreateCommand())
                {
                    try
                    {
                        ...
                        using (SqlDataReader objSDR = objCmd.ExecuteReader())
                        {
                            ...
                            if (!objSDR["LinkdLIID"].Equals(DBNull.Value))
                                entContact.LinkdINID = Convert.ToString(objSDR["LinkdINID"]);
            ...
            using (SqlConnection objConn = new SqlConnection(ConnectionString))
            {
                ...
                using (SqlCommand objCmd = objConn.CreateCommand())
                {
                    try
                    {
                        ...
                        objCmd.Parameters.AddWithValue("@ContactFilePath", entContact.ContactFilePath);
                        objCmd.Parameters.AddWithValue("@FileSize", entContact.FileSize);
                        objCmd.Parameters.AddWithValue("@FileType", entContact.FileType);
